...
struct BBox {
  ...
  vector<PixelLoc> pixelsInside() const { vector<PixelLoc> res;
    ...
    for ( pos.x = minX(); pos.x <= maxX(); ++pos.x )
      res.push_back(pos);
    return res; }
  void draw(bool inner=false) const;
...
struct Image {  // matrix we can draw on the screen
...
namespace Wright {
  ...
  extern string selectedTileID, selectedCornerID;
  extern int numPoly3ToLoad;
  extern char **poly3ToLoad;
};
...
public:
  ...
  bool isStub = false;  // stubs are contours that have not been fully loaded
...
public:
  ...
  void addEmptyContour(const string &imgName, bool isStub=false);
...
struct Coord3 {
  ...
  Coord3(istream &is) { is >> x;
      is >> ws; assert(is.get() == ','); is >> y;
      is >> ws; assert(is.get() == ','); is >> z; }

  double dot(Coord3 c) const {
    return x*c.x + y*c.y + z*c.z; }
  double operator*(Coord3 c) const {
    return dot(c); }
  ...
  void moveToward(Coord3 intersection, double howMuch);
  bool closeTo(Coord3 b, double thresh=0.00001) const {
    return dist2(*this, b) < thresh; }
  void draw() const;
  ...
  friend Coord3 operator-(Coord3 a, Coord3 b) {
    return Coord3(a.x - b.x, a.y - b.y, a.z - b.z); }
  friend Coord3 operator*(double s, Coord3 c) {
    return Coord3(s*c.x, s*c.y, s*c.z); }
  friend Coord3 operator/(Coord3 c,double s) {
    assert(s != 0);
    return Coord3(c.x/s, c.y/s, c.z/s); }
  friend double dist2(Coord3 a, Coord3 b) {
    double dx = a.x - b.x, dy = a.y - b.y, dz = a.z - b.z;
    return dx*dx + dy*dy + dz*dz; }
  friend double dist(Coord3 a, Coord3 b) {
    return sqrt(dist2(a,b)); }
  friend bool isNoCoord3(Coord3 c) {
    return (c.x==1e10) && (c.y==1e10) && (c.z==1e10); }  // must equal noCoord3
  friend Coord3 cross(Coord3 a, Coord3 b) {
    return Coord3( a.y*b.z - a.z*b.y,
                   a.z*b.x - a.x*b.z,
                   a.x*b.y - a.y*b.x ); }
  friend ostream & operator<<(ostream& ostr, const Coord3& c) {
    ostr << c.x << "," << c.y << "," << c.z;
    return ostr; }
};
static const Coord3 noCoord3(1e10, 1e10, 1e10); // see isNoCoord3 above
...
struct Quaternion {
  ...
  Quaternion(double rotAngle, Coord3 axis);
  explicit Quaternion(Coord3 eulerRot);

  Quaternion operator*(Quaternion c);
  double mag() const;
  void normalize();
  void invert() { w *= -1; }
  Coord3 rotatePoint(Coord3) const;
  Coord3 asEuler() const;
...
#define USE_EULER 0
struct Camera {
  Coord3 pos;        // the focal point in world coordinates
#if USE_EULER
  Coord3 rot;        // euler angles: rot.x = rotate about world x-axis, y, z
#else
  Quaternion qrot;
#endif
  Coord focalLength, principalPt;
  vector <double> K, Kchan[3];     // radial distortion parameters (per channel)
  vector <double> P, Pchan[3];     // tangential distortion parameters

 public:
  Camera() : pos(noCoord3) {}
  Camera( Coord3 _pos, Coord3 _rot, Coord _focal, Coord _prin ) :
    pos(_pos),
#if USE_EULER
    rot(_rot),
#else
    qrot(_rot),
#endif
    focalLength(_focal), principalPt(_prin) {}
  Camera( const char * );
  Camera( istream & );

  bool chromatic() const { return !Kchan[0].empty(); }
  Coord view(Coord3 viewed, int chan=-1, bool suppressWarning=false) const;
  Coord3 view3(Coord3 viewed, int chan=-1, bool suppressWarning=false) const;  // include depth info
  Coord3 toward(Coord imagePt, int chan=-1) const;
  Coord toward2(Coord imagePt, int chan=-1) const;  // undistortion in image
  Coord3 transWC(Coord3 world) const;
  Coord3 transCW(Coord3 cameraCoord) const;
  Coord distortPoint(Coord undist, int chan=-1) const;
  Coord undistortPoint(Coord distorted, int chan=-1) const;
  Matrix3x3 getRotation() const;
#if USE_EULER
  void setJustRotation(Coord3 newRot);  // maintain optical center
#else
  void setJustRotation(Quaternion newRot);  // maintain optical center
#endif
  void setOpticalCenter(Coord3 newPos); // maintain rotation
  void moveForwardBy(double d, Coord dir=noCoord);
  void moveLeftBy(double d);
  void moveUpBy(double d);
  Coord3 opticalCenter() const;
  Coord3 viewDir() const;
  double distAlongOpticalAxis(Coord3 pt) const;
  vector<Coord3> imagePlaneCorners() const;
#if USE_EULER
  void applyXform(Coord3 offset, Coord3 turn);  // change extrinsics
  void applyInverseXform(Coord3 offset, Coord3 turn);  // change extrinsics
#else
  void applyXform(Coord3 offset, Quaternion turn);  // change extrinsics
  void applyInverseXform(Coord3 offset, Quaternion turn);  // change extrinsics
#endif
};
ostream& operator<<(ostream &os, const Camera &);
double undistortionMinFunc(double vect[]);
Coord3 applyEuler(Coord3 rot, Coord3 pt);
pair<Coord, Coord> findEpipolarLine( const Camera &c0, Coord ptInC0, const Camera &c1 );

struct Poly3 {  // represents a collection of polygons in 3d
  vector<Coord3> vert;
  vector<Coord> imgVert;  // for texture mapping
  vector< vector<int> > face;
  bool selected = false, fill = true, stroke = false;
  Coord3 trans, rot;
  double scale = 1.;
  int selectedVert[4] = {-1, -1, -1, -1};  // allow selection of four vertices,
                                           //  for alignment
  vector<Color> vertColor; // allow color to be applied per-vertex
  string id;               // associated tile
  vector<string> imgName;  // for texture mapping, per face

  Poly3() {}
  Poly3( const char * );
  Poly3( istream &is, bool oneLine=false );

  Coord3 nearestPointOnPolygon(int index, Coord3 query) const;
  double distToPolygon(int index, Coord3 query) const;
  vector<int> fastNearPolygons(Coord3 query) const;
  Coord3 fastNearestPoint(Coord3 query) const;
  double distTo(Coord3 query) const;

  Coord3 worldVertex(int) const;
  bool isSelectedVert(int) const;
  Poly3 draw(const Camera &, bool createProjCopy=false) const;
  void print(ostream &) const;
  void printPoly3(ostream &, bool oneLine=false) const;
  void outputProjected(ostream &, const Camera &) const;
  void output3D(ostream &) const;
};
ostream& operator<<(ostream &, const Poly3 &);
...
bool isDigit(char c);
bool isAlpha(char c);
void loadCorrespFile(const char *);
Camera getIthCamera(int i);
vector<string> getTileIDsForImage(const string &camName);
vector<Poly3> &loadPolysForImage(const string &imgName);
vector<PixelLoc> getContour(const string &tileID, const string &imgName, bool removeSpecks=true);
vector< vector<Coord> > getContourBoundary(const string &tileID, const string &imgName);
Image getContourColors(const string &tileID, const string &imgName, bool removeSpecks=true);
void addOverlayPoint(Coord3 a, Color c=RED);
void addOverlaySegment(Coord3 a, Coord3 b, Color c=RED);
void addOverlayText(Coord3 a, const string &s, Color c=BLACK);
void addOverlayPolygon(const vector<Coord3> &pt, Color c=BLUE);
vector<Coord> getFeaturePoints(const string &tileID, const string &imgName);
Matrix3x3 getHomography(const string &tileID, const string& imgName0, const string &imgName1);
